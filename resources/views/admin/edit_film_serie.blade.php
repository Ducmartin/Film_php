...
@section('content')
    <div class="col-md-10">
        <div id="content">
            ...
            <form method="POST" action="{{ route('validation_edit_film_series',$id) }}" enctype="multipart/form-data"
                file='true'>
                @csrf
                @csrf
                <div class="form-group title">
                    <label for="title">Tên Phim:</label>
                    <input type="text" class=" form-control" value="{{ $film->name }}" name="title" id="title"
                ...
                <div class="form-group rate">
                    ...
                    <input type="text" min="1" max="10" class=" form-control" value="{{ $film->rate }}" name="rate"
                ...
                <div class="form-group rate">
                    <label for="end_episode">Tổng số tập phim:</label>
                    <input type="number" min="1" class=" form-control" value="{{ $film->end_episode }}" id="end_episode"
                        name="end_episode" placeholder="Tổng số tập phim">
                    @error('end_episode')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group rate">
                    <label for="showtime">Lịch chiếu phim:</label>
                    <input type="text" class=" form-control" id="showtime" value="{{ $film->showtime }}" name="showtime"
                        placeholder="Lịch chiếu phim">
                    @error('showtime')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="country_id" class="form-control" id="country_id">
                        ...
                        @foreach ($countries as $item)
                            <option {{ $id = $item->id == $film->country_id ? 'selected' : '' }} value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('country_id')
