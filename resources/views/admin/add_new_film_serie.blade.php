...
<div class="col-md-10">
    <div id="content">
        ...
        <form method="POST" action="{{route('validation_film_series')}}" enctype="multipart/form-data" file='true'>
           @csrf
        ...
        <div class="form-group rate">
            ...
            <input type="text" class=" form-control" id="showtime" name="showtime" placeholder="Lịch chiếu phim">
            @error('showtime')
            <small class="form-text text-danger">{{$message}}</small> 
            @enderror
        </div>
        <div class="form-group">
           ...
           <div class="form-check">
                <input type="checkbox" style="cursor: pointer;" value="{{$kind->slug}}"  name="kind_id[]" id="{{$kind->id}}" >
        ...
        <div class="form-group bg_img">
            ...
            <input type="file"  name="bg_img" id="bg_img" placeholder="Ảnh nền phần giới thiệu">
            @error('bg_img')
            <small class="form-text text-danger">{{$message}}</small> 
            @enderror
        </div>
